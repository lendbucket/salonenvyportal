...
const inputStyle: React.CSSProperties = {
  ...
  backgroundColor: "rgba(26,19,19,0.03)", border: "1px solid rgba(26,19,19,0.08)",
  ...
  return (
    <div style={{ padding: "clamp(16px,4vw,28px)", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        {filtered.map(po => (
          <div key={po.id} onClick={() => setSelected(po)} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
              <div>
                <span style={{ color: "#1A1313", fontSize: "14px", fontWeight: 700 }}>{po.poNumber}</span>
                <span style={{ color: "rgba(26,19,19,0.5)", fontSize: "12px", marginLeft: "12px" }}>{po.supplier}</span>
                <span style={{ color: "rgba(26,19,19,0.4)", fontSize: "11px", marginLeft: "12px" }}>{po.location.name}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <span style={{ color: "rgba(26,19,19,0.5)", fontSize: "12px" }}>${po.totalAmount.toFixed(2)}</span>
                <span style={{
        ...
        {filtered.length === 0 && (
          <div style={{ ...cardStyle, textAlign: "center", color: "rgba(26,19,19,0.4)" }}>No purchase orders found</div>
        )}
      </div>
      ...
      {selected && (
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.25)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: "20px" }}
          ...
          <div style={{ ...cardStyle, maxWidth: "600px", width: "100%", maxHeight: "80vh", overflowY: "auto", borderRadius: 16, boxShadow: "0 8px 32px rgba(0,0,0,0.12), 0 2px 8px rgba(0,0,0,0.08)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: "13px", color: "rgba(26,19,19,0.6)", marginBottom: "16px" }}>
