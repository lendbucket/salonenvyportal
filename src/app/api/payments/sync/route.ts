...
export async function POST() {
  ...
  const existing = await prisma.syncJob.findFirst({ where: { type: "payments", status: { in: ["pending", "running"] } } })
  if (existing) return NextResponse.json({ jobId: existing.id, status: existing.status, message: "Payments sync already in progress" }, { status: 409 })
  const userId = (session.user as Record<string, unknown>).id as string
  const job = await prisma.syncJob.create({ data: { type: "payments", status: "running", triggeredBy: userId } })
...
export async function GET() {
  ...
  if ((session.user as Record<string, unknown>).role !== "OWNER") return NextResponse.json({ error: "Forbidden" }, { status: 403 })
  const { prisma } = await import("@/lib/prisma")
  const job = await prisma.syncJob.findFirst({ where: { type: "payments" }, orderBy: { startedAt: "desc" } })
