...
import { processOrdersSyncBatch } from "@/lib/sync/orders-worker"
...
export async function GET(req: NextRequest) {
  ...
  try {
    const job = await prisma.syncJob.findFirst({ where: { type: "orders", status: "running" }, orderBy: { startedAt: "asc" } })
