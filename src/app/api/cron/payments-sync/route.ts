...
import { processPaymentsSyncBatch } from "@/lib/sync/payments-worker"
...
export async function GET(req: NextRequest) {
  ...
  try {
    const job = await prisma.syncJob.findFirst({ where: { type: "payments", status: "running" }, orderBy: { startedAt: "asc" } })
    ...
    const result = await processPaymentsSyncBatch(job.id)
